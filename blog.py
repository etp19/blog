...
class BlogFront(BlogHandler):
    def get(self):
        ...
        #user_for_post = Post.query(Post.user_post == User).get()
        ...
        if number1_button:
            ...
            if self.user:
                if not self.user.key == post.user_post:
                    ...
                    comments = ndb.gql("select * from Reply where post_info = :1 order by created desc limit 10", key)
                    like = ndb.gql("select * from Likes where post_info= :1", key)
                    self.render("permalink.html", post=post, comments=comments, error_comment=error_like, like=like)
        elif delete_request:
                post.key.delete()
                self.redirect('/blog')
